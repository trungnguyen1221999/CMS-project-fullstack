...
namespace Application.Services.Royalty
{
    public class RoyaltyService : IRoyaltyService
    {
        ...
        )
        {
            ...
            var hasRoyaltyReportViewPermission =
                _permissionService.HasRoyaltyReportViewPermission(currentUserId);

            if (request.UserId != currentUserId && !hasRoyaltyReportViewPermission)
            {
